...
get '/menu/:action' => sub{
  ...
  if($action eq 'products'){
    ...
    my $instructions=$dbh->selectall_hashref("SELECT ROUTER3.ID,RECIPE.ID AS RECIPEID,RECIPE.NAME AS RECIPE,INSTRUCTIONS.STEP,ROUTER2.YIELD,INSTRUCTIONS.DESCRIBE,INGREDIENTS.NAME AS INGREDIENT,SUM(ROUTER3.VALUE*ROUTER2.YIELD) AS VALUE,INSTRUCTIONS.WAIT FROM ROUTER2 RIGHT JOIN RECIPE ON ROUTER2.RECIPEID=RECIPE.ID INNER JOIN INSTRUCTIONS ON RECIPE.ID=INSTRUCTIONS.RECIPEID LEFT JOIN ROUTER3 ON INSTRUCTIONS.ID=ROUTER3.INSTRUCTIONID LEFT JOIN INGREDIENTS ON ROUTER3.INGREDIENTID=INGREDIENTS.ID GROUP BY RECIPE.ID, ROUTER3.INSTRUCTIONID,ROUTER3.INGREDIENTID",'ID');
    my $ascsort=$dbh->selectall_arrayref("SELECT ROUTER3.ID FROM ROUTER2 RIGHT JOIN RECIPE ON ROUTER2.RECIPEID=RECIPE.ID INNER JOIN INSTRUCTIONS ON RECIPE.ID=INSTRUCTIONS.RECIPEID LEFT JOIN ROUTER3 ON INSTRUCTIONS.ID=ROUTER3.INSTRUCTIONID LEFT JOIN INGREDIENTS ON ROUTER3.INGREDIENTID=INGREDIENTS.ID GROUP BY RECIPE.ID, ROUTER3.INSTRUCTIONID,ROUTER3.INGREDIENTID");

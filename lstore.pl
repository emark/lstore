...
get '/recipe/:action/:id' => sub{
  ...
  if($action eq 'view')
  {
    ...
    my $instructions=$dbh->selectall_hashref("SELECT ROUTER3.ID,INGREDIENTS.NAME AS INGREDIENT,INSTRUCTIONS.STEP,INSTRUCTIONS.WAIT,INSTRUCTIONS.DESCRIBE,ROUTER3.VALUE,MEASURE.NAME AS MEASURE FROM INSTRUCTIONS RIGHT JOIN ROUTER3 ON INSTRUCTIONS.ID=ROUTER3.INSTRUCTIONID LEFT JOIN INGREDIENTS ON ROUTER3.INGREDIENTID=INGREDIENTS.ID LEFT JOIN MEASURE ON INGREDIENTS.MEASUREID=MEASURE.ID WHERE INSTRUCTIONS.RECIPEID=$id ORDER BY INSTRUCTIONS.STEP",'ID',,);
    ...
    if($id eq '0'){
      my @recipe=$self->param('recipe');#Параметры нового рецепта
      my @ingredients=$self->param('ingredients');
      my @value=$self->param('value');
      $dbh->do("INSERT INTO RECIPE(NAME,RDESCRIBE,WEIGHT) VALUES(\"$recipe[0]\",\"$recipe[1]\",$recipe[2])");
      my $recipeid=$dbh->last_insert_id(undef,undef,'RECIPE','ID');
      for (my $n=0;$n<@ingredients;$n++){
	if($value[$n]){
...
get '/menu/:action' => sub{
  ...
  if($action eq 'products'){
    ...
    my $ascsort=$dbh->selectall_arrayref('SELECT RECIPE.ID FROM ROUTER2 LEFT JOIN RECIPE.ID=ROUTER2.RECIPEID ORDER BY RECIPE.ID');
    $self->stash(ingredients=>$ingredients,
		 recipes=>$recipes,
		 );

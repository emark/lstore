...
get '/recipe/:action/:id' => sub{
  ...
  if($action eq 'view')
  {
    ...
    my $instructions=$dbh->selectall_hashref("SELECT INSTRUCTIONS.ID,INSTRUCTIONS.STEP,INSTRUCTIONS.WAIT,INSTRUCTIONS.DESCRIBE,INSTRUCTIONS.VALUE FROM INSTRUCTIONS WHERE INSTRUCTIONS.RECIPEID=$id ORDER BY INSTRUCTIONS.STEP",'ID');
